...
14. **キラキラ動画のCanvas転写は30fps制限+タブ復帰時の再生再開つき**(main.js)。iOSはタブ切替で動画を止めるが復帰時に再開しない(エフェクトが固まる)。また60fps転写はiPhoneが発熱する。制限を外したり復帰処理を消したりしないこと。
...
## iOS Safari対応(調査済み・再調査不要。最終仕様=2026-07-18確定)

**Safariの描画仕様(実機検証で確定した事実):**
- ステータスバー/下部バーの裏には「スクロールする本文レイヤー」だけが描画される。**fixed要素の画面外部分・疑似要素は描画されない**(ナビをtop:-100pxしても::afterで帯を足しても上端は塞げなかった)
- fixed要素のサイズは「バー展開時の小さい画面」基準で固定。バーが格納されると下に未カバー領域(約180pt)が生じる
- 下部バー自体の半透明の透けはSafariの演出でページ側から制御不可(全サイト共通)
- viewport-fit=cover + env() は通常ブラウズ縦持ちでは実質効かない(PWA/横持ち用に付けてある)

**最終的に採用した実装(style.css / index.html):**
1. **上端の貫通対策**: `.ios-top-cover`(bodyの先頭、`position:sticky;top:0`+::beforeで上に150pxの黒帯)。stickyは本文レイヤー側なのでバー裏にも描画される。**このためにbodyは`overflow-x:clip`**(hiddenだとstickyが死ぬ。hiddenはフォールバックとして併記)
2. **メニュー**: iOS限定(`@supports(-webkit-touch-callout:none)`)で`inset:-120px 0 -220px`に延長し、バーの全状態で全画面をカバー。上120pxは黒(sticky帯と連続)、本体はrgba(4,6,15,.72)+blur(20px)の均一(フェードは試行の末廃止)
3. **メニュー展開中**(`html.menu-open`、main.jsのsetMenuが付与): ナビの背景・罫線・ブラーを消してオーバーレイ1枚に統一(半透明の二重掛けの段差防止)
4. ナビのiOS用`top:-100px`+padding補正は残置(害なし)。スクロール時ナビは全環境共通でrgba(4,6,15,.8)+blur(10px)——**これ以上不透明にしない**(ユーザーが透け感を確定済み)
